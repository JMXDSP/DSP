...
def response(flow):
    ...
    #                     # endregion
    #
    #                     sSql = "INSERT INTO dy_aweme(uid, aweme_id, str_desc, share_url, create_time, comment_count, digg_count, download_count, forward_count, share_count, play_music,owner_nickname,origin_cover,cover,s) " \
    #                            "VALUES ('{}', '{}', '{}', '{}', '{}', {}, {}, {}, {}, {}, '{}','{}','{}','{}','{}')".format(
    ...
    #                     # endregion
    #
    #                     sSql = "INSERT INTO dy_user(uid, user_id, user_name, birthday, aweme_count, favoriting_count, follower_count, following_count, total_favorited, user_gender, signature, s) " \
    #                            " VALUES ('{}', '{}', '{}', '{}', {}, {}, {}, {}, {}, {}, '{}', '{}')".format(
    ...
    if 'aweme/v1/discover/search/' in flow.request.url:
        try:
            for aweme in json.loads(flow.response.text)['user_list']:
                user_info = user = aweme['user_info']
                user_id = user_info['unique_id']

                user_sql = "insert into dy_dyh1(dyh) values ('{}')".format(user_id)
                print(user_sql)
...
# 11819201
    ...
    if 'aweme/v1/user/' in flow.request.url:
        ...
        try:
            ...
            region= user.get('region')  # 国家缩写
            province= user.get('province')  # 省
            city= user.get('city')  # 市
            avatar_larger = user['avatar_larger']['url_list'][0]  # 头像 1080*1080
            ...
            user_sql = "INSERT INTO dy_user(uid,user_id,user_name,birthday,aweme_count,favoriting_count,follower_count,following_count,total_favorited,user_gender,signature) VALUES('" + str(
    ...
    if 'aweme/v1/aweme/post/' in flow.request.url:
        for aweme in json.loads(flow.response.text)['aweme_list']:
            try:
                ...
                play_music = aweme['music']['play_url']['uri']  # 背景音乐
                owner_nickname = aweme['music']['owner_nickname']  # 背景音乐名称
                origin_cover = aweme['video']['origin_cover']['url_list'][0]  # 默认图片 视频默认显示
                cover = aweme['video']['cover']['url_list'][0]  # 默认图片 视频中
                duration = aweme['video']['duration']  # 视频时间

                aweme_sql = "insert into dy_aweme(uid,aweme_id,str_desc,share_url,create_time,comment_count,digg_count,download_count,forward_count,share_count,play_music,owner_nickname,origin_cover,cover,duration) values "
    ...
    if '/aweme/v2/comment/list/' in flow.request.url:
        for comment in json.loads(flow.response.text)['comments']:
            ...
            try:
                ...
                # endregion
                sSql = "insert into dy_comments(cid,aweme_id,create_time,digg_count,reply_comment_total,status,stick_position,text,user_nickname,user_uid,is_author_digged,reply_id,reply_to_reply_id,user_digged" \
                       ",user_avatar_168x168,user_avatar_300x300,user_avatar_larger,user_avatar_uri,user_birthday,user_signature,user_gender,user_city) values " \
                       ...
                       ",'%s','%s','%s','%s','%s','%s','%s','%s')" % (
                           ...
                           user_signature, user_gender, user_city
                       )
                print(sSql)
                conn.ping(reconnect=True)
                cursor.execute(sSql)
